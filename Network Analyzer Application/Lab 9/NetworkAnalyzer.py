...
class NetworkAnalyzer:
    ...
    def protocols_by_connections(self):
        protocols_by_connection_dict = dict()

        for item in self._data:
            source_ip = item[4]
            protocols_by_connection_dict[source_ip] = protocols_by_connection_dict.get(source_ip, 0) + 1

        protocols_by_connection_list = list(protocols_by_connection_dict.items())

        protocols_by_connection_sorted = self.quicksort(protocols_by_connection_list, 0,
                                                        len(protocols_by_connection_list) - 1, 1)

        return protocols_by_connection_sorted
    ...
    def protocols_by_bytes(self):
        ...
        for item in self._data:
            source_ip = item[4]
            protocols_by_bytes_dict[source_ip] = protocols_by_bytes_dict.get(source_ip, 0) + int(item[5])
    ...
    def connections_by_connections(self):
        ...
        for item in self._data:
            source_ip = item[2]
            destination_ip = item[3]
            ips = (source_ip, destination_ip)
            connections_by_connections_dict[ips] = connections_by_connections_dict.get(ips, 0) + 1

        connections_by_connections_list = list(connections_by_connections_dict.items())

        connections_by_connections_sorted = self.quicksort(connections_by_connections_list, 0,
                                                           len(connections_by_connections_list) - 1, 1)

        return connections_by_connections_sorted

    #
    def connections_by_bytes(self):
        connections_by_bytes_dict = dict()

        for item in self._data:
            source_ip = item[2]
            destination_ip = item[3]
            ips = (source_ip, destination_ip)
            connections_by_bytes_dict[ips] = connections_by_bytes_dict.get(ips, 0) + int(item[5])

        connections_by_bytes_list = list(connections_by_bytes_dict.items())

        connections_by_bytes_sorted = self.quicksort(connections_by_bytes_list, 0, len(connections_by_bytes_list) - 1,
                                                     1)

        return connections_by_bytes_sorted
